"""
Test: trainer sees ALL GPUs (no CUDA_VISIBLE_DEVICES),
vLLM gets restricted CUDA_VISIBLE_DEVICES as before.
The key insight: the trainer process must see the same physical GPUs
as the vLLM workers for NCCL bootstrap to work.

Usage:
    python tmp-scripts/test_nccl_no_cvis.py --vllm-gpus 2,3 --trainer-gpu 0
"""
...
def main():
    ...
    n_dp = len(args.vllm_gpus.split(","))

    print(f"Trainer GPU: {args.trainer_gpu} (no CUDA_VISIBLE_DEVICES restriction)")
    print(f"vLLM GPUs:   {args.vllm_gpus} ({n_dp} DP, via CUDA_VISIBLE_DEVICES)")
    ...
    vllm_cmd = [
        ...
        "--disable-log-requests", "--data-parallel-size", str(n_dp),
    ]
    ...
    print(f"Starting vLLM on GPUs {args.vllm_gpus} (port {port})...")
    ...
    try:
        ...
        # Trainer: NO CUDA_VISIBLE_DEVICES restriction, just set_device
