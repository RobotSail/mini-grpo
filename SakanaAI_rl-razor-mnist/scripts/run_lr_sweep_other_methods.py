#!/usr/bin/env python3
"""LR sweep for sft1, sft2, oracle, grpo_kl in fp32. 15 LRs, 30 seeds, cosine warmup, 8 GPUs."""
...
OUTPUT = Path("experiments/lr_sweep_other_methods_v2")
N_GPUS = 8
WORKERS_PER_GPU = 10
...
def run_one(method, lr, base_model, base_sd, train_loader, pl, fl, seed, device, steps_per_epoch):
    set_seed(seed)
    model = MLP().to(device)
...
def worker(gpu_id, jobs, base_sd, results_dict):
    ...
    for method, lr, seed in jobs:
        par, fash, kl = run_one(method, lr, base, base_sd, tl, pl, fl, seed, device, steps_per_epoch)
        key = f"{method}_lr{lr:.2e}_seed{seed}"
        results_dict[key] = {
            "parity": par, "fashion": fash, "kl": kl,
            "lr": lr, "method": method, "seed": seed,
        }
...
if __name__ == "__main__":
    ...
    for method in METHODS:
        for lr in LRS:
            for seed in SEEDS:
                all_jobs.append((method, lr, seed))
    ...
    out_path = OUTPUT / "lr_sweep_other_methods_results.json"
