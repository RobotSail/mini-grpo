#!/usr/bin/env python3
"""Magnitude threshold sweep for sft1, sft2, oracle, grpo_kl.

Cosine warmup scheduler, 26 thresholds + bf16/fp32 baselines, 30 seeds, 8 GPUs.
"""
...
OUTPUT = Path("experiments/threshold_other_methods_v2")
N_GPUS = 8
WORKERS_PER_GPU = 10
...
def run_one(method, mode, threshold, base_model, base_sd, train_loader, pl, fl, seed, device, steps_per_epoch):
    set_seed(seed)
    model = MLP().to(device)
...
def worker(gpu_id, jobs, base_sd, results_dict):
    ...
    for method, mode, threshold, seed in jobs:
        par, fash, kl = run_one(method, mode, threshold, base, base_sd, tl, pl, fl, seed, device, steps_per_epoch)
        if mode == "threshold":
            key = f"{method}_thresh_{threshold:.2e}_seed{seed}"
        else:
            key = f"{method}_{mode}_seed{seed}"
        results_dict[key] = {
            "parity": par, "fashion": fash, "kl": kl,
            "threshold": threshold, "mode": mode, "method": method, "seed": seed,
        }
...
if __name__ == "__main__":
    ...
    for method in METHODS:
        for threshold in THRESHOLDS:
            for seed in SEEDS:
                all_jobs.append((method, "threshold", threshold, seed))
        for seed in SEEDS:
            all_jobs.append((method, "bf16", 0.0, seed))
            all_jobs.append((method, "fp32", 0.0, seed))
    ...
    out_path = OUTPUT / "threshold_other_methods_results.json"
