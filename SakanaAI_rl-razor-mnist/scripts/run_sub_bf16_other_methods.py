#!/usr/bin/env python3
"""Batched runs for sft1, sft2, oracle, grpo_kl with sub-bf16 mantissa (m1-m6), 5 seeds each.

4 methods × 6 mantissa × 5 seeds = 120 jobs.
"""
...
OUTPUT = "experiments/sakana_sub_bf16_all5"
N_GPUS = 8
PER_GPU = 10
...
for batch_start in range(0, len(all_jobs), batch_size):
    ...
    procs = []
    for i, (method, mbits, mlabel, seed, run_name) in enumerate(batch):
        gpu_id = i % N_GPUS
        cmd = [
