#!/usr/bin/env python3
"""
Plot accuracy vs training progress for nvme2 checkpoint experiments.
Creates separate plots for GRPO and SFT experiments.
"""
...
def format_tokens(x: float) -> str:
    """Format token count for labels."""
    if x >= 1e6:
        return f"{x/1e6:.2f}M"
    elif x >= 1e3:
        return f"{x/1e3:.0f}K"
...
def format_samples(x: float) -> str:
    """Format sample count for labels."""
    if x >= 1e3:
        return f"{x/1e3:.1f}K"
    return f"{x:.0f}"
...
def plot_grpo(base_dir: Path, output_path: Path):
    ...
    experiments = {
        "adamw": {
            "file": base_dir / "qwen2-1.5b-gsm8k-grpo-adamw_verify_1/eval_results.json",
            "color": "#1f77b4",
            "marker": "o",
            "label": "AdamW + GRPO",
            "offset": (5, 5),
        ...
        "muon": {
            "file": base_dir / "qwen2-1.5b-gsm8k-grpo-muon_verify_1/eval_results.json",
            "color": "#d62728",
            "marker": "D",
            "label": "Muon + GRPO",
            "offset": (5, -10),
    ...
    ax.set_title("GRPO: AdamW vs Muon - GSM8K Accuracy", fontsize=14)
...
def plot_sft(base_dir: Path, output_path: Path):
    ...
    experiments = {
        "adamw": {
            "file": base_dir / "qwen2-1.5b-gsm8k-sft-adamw_verify_1/eval_results.json",
            "color": "#1f77b4",
            "marker": "o",
            "label": "AdamW + SFT",
            "offset": (5, 5),
        ...
        "muon": {
            "file": base_dir / "qwen2-1.5b-gsm8k-sft-muon_verify_1/eval_results.json",
            "color": "#ff7f0e",
            "marker": "^",
            "label": "Muon + SFT",
            "offset": (5, -10),
    ...
    ax.set_ylabel("GSM8K Accuracy", fontsize=12)
    ax.set_title("SFT: AdamW vs Muon - GSM8K Accuracy", fontsize=14)
    ax.legend(loc="lower right", fontsize=10)
    ax.grid(True, alpha=0.3)
    ax.set_ylim(0, 1.0)
...
def main():
    base_dir = Path("/mnt/nvme2n1/checkpoints")

    # Create GRPO plot
    grpo_output = base_dir / "grpo_accuracy_plot.png"
    plot_grpo(base_dir, grpo_output)

    # Create SFT plot
    sft_output = base_dir / "sft_accuracy_plot.png"
