#!/usr/bin/env python3
"""
Plot accuracy vs training progress for v1 artifacts (v2 eval results).
...
def plot_grpo(base_dir: Path, output_path: Path):
    ...
    experiments = {
        "adamw": {
            "file": base_dir / "grpo_adamw/eval_results_v2.json",
            "color": "#1f77b4",
            "marker": "o",
            "label": "AdamW + GRPO",
            "offset": (5, 5),
        ...
        "muon": {
            "file": base_dir / "grpo_muon/eval_results_v2.json",
            "color": "#d62728",
            "marker": "D",
            "label": "Muon + GRPO",
            "offset": (5, -10),
    ...
    ax.set_title("GRPO: AdamW vs Muon - GSM8K Accuracy (v2 Eval)", fontsize=14)
...
def plot_sft(base_dir: Path, output_path: Path):
    ...
    experiments = {
        "adamw": {
            "file": base_dir / "sft_adamw/hf_format/eval_results_v2.json",
            "color": "#1f77b4",
            "marker": "o",
            "label": "AdamW + SFT",
            "offset": (5, 5),
        ...
        "muon": {
            "file": base_dir / "sft_muon/hf_format/eval_results_v2.json",
            "color": "#ff7f0e",
            "marker": "^",
            "label": "Muon + SFT",
            "offset": (5, -10),
    ...
    ax.set_ylabel("GSM8K Accuracy", fontsize=12)
    ax.set_title("SFT: AdamW vs Muon - GSM8K Accuracy (v2 Eval)", fontsize=14)
    ax.legend(loc="lower right", fontsize=10)
    ax.grid(True, alpha=0.3)
    ax.set_ylim(0, 1.0)
...
def main():
    ...
    # Create GRPO plot
    grpo_output = output_dir / "grpo_accuracy_plot_v2.png"
    plot_grpo(base_dir, grpo_output)

    # Create SFT plot
    sft_output = output_dir / "sft_accuracy_plot_v2.png"
